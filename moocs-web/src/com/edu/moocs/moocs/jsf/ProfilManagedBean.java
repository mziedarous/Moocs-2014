	...
	public boolean notifi() {
		...
		if (texts != null) {
			for (String text : texts) {
				...
				FacesMessage context = new FacesMessage(
						FacesMessage.SEVERITY_INFO, "notifications",
						" we have :  " + text + " as a new course");
		...
		if (FacesContext.getCurrentInstance().getCurrentPhaseId() == PhaseId.RENDER_RESPONSE) {
			...
			try {
				InputStream stream = new BufferedInputStream(
						new FileInputStream("C:\\tmp\\" + user.getLogin()
								+ ".jpg"));
				...
				InputStream stream = new BufferedInputStream(
						new FileInputStream("C:\\tmp\\default.jpg"));
